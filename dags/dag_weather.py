...
task1 = BashOperator(
    task_id='task1',
    bash_command='python3 /root/airflow/scripts/task1.py --date {{ ds }} ' +f'--host {connection.host} --dbname {connection.schema} --user {connection.login} --jdbc_password {connection.password} --port 5432',
    dag=dag)
